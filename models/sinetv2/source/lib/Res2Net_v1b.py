...
class Bottle2neck(nn.Module):
    ...
    def forward(self, x):
        ...
        if (
            ...
            self.scale != 1
            and self.stype == 'stage'
        ):

            out = torch.cat(
                (
                    out,
                    self.pool(spx[self.nums])
                ),
                1
            )
...
def res2net50_v1b_26w_4s(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        project_sinetv2_dir = os.path.dirname(
            os.path.dirname(
                os.path.dirname(
                    current_file
                )
