# training/train_classifier_fixed.py
import os
import sys
...
class CropDataset(Dataset):
    def __init__(self, split='Train', transform=None):
        ...
        print(f"{split}: {len(self.image_files)} crops loaded")
    ...
    def __getitem__(self, idx):
        ...
        label = self.labels[basename] - 1  # 0-indexed
        if self.transform:
...
def train():
    print("=" * 70)
    print("CLASSIFIER TRAINING - FIXED VERSION")
    print("=" * 70)
    ...
    for epoch in range(30):
        ...
        # Early stopping
        if epoch > 5 and val_acc < best_val_acc - 1.0:
